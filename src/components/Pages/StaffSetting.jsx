...
const StaffSetting = () => {
  // Predefine three rows for Commission Slabs
  ...
  // Remove the staff fetching code

  // Function to handle changes to commission slabs
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update state with API response to reflect in the form fields
        if (result.status) {
          // Update commission slabs
          const updatedSlabs = result.data.slab_data.map((slab, index) => ({
            id: index + 1,  // Adding id field for the table row key
            staff_slab: slab.staff_slab,
            staff_commision_cap: slab.staff_commision_cap,
            staff_target_business: slab.staff_target_business,
          }));
          ...
          // Save month days to localStorage
